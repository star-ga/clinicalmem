...
2. **Recall gate (PCCP):** **100% recall** on every severity class
   represented in the 118-pair OpenEvidence-cited cohort —
   contraindicated (23/23), major (4/4), serious (69/69), moderate
...
#    major / serious / moderate against the 118-pair OpenEvidence-cited
#    cohort.
...
Optional: full pytest suite — `python3 -m pytest tests/test_engine/ tests/test_scripts/ -q`. Should report **1046+ passed**.
...
|---|---|
| `100% recall · contraindicated · 23/23` | `docs/openevidence_cache.json` (118 entries) → `docs/pccp_eval_latest.json` (per-pair verdicts) |
...
| `27 synthetic patients · 44 NPIs` | `docs/synthea_demo_cohort.json` (FHIR R4 bundle, all NPIs Luhn-valid). **Per-patient drug-pair → cache-entry traceability matrix:** `docs/cohort_coverage_matrix.md` (regenerate with `scripts/build_cohort_coverage.py`). |
