...
2. **Recall gate (PCCP):** **100% recall** on every severity class
   represented in the 123-pair OpenEvidence-cited cohort —
   contraindicated (28/28), major (4/4), serious (69/69), moderate
...
#    major / serious / moderate against the 123-pair OpenEvidence-cited
#    cohort.
...
Optional: full pytest suite — `python3 -m pytest tests/test_engine/ tests/test_scripts/ -q`. Should report **1117+ passed**.
...
|---|---|
| `100% recall · contraindicated · 28/28` | `docs/openevidence_cache.json` (123 entries) → `docs/pccp_eval_latest.json` (per-pair verdicts) |
...
| `Layer 4.5 BitNet confusion matrix (live deployment)` | `docs/bitnet_confusion_matrix.json` (regenerate with `scripts/build_bitnet_confusion_matrix.py`) — full ground-truth × predicted matrix on the 123-pair cache, plus per-class precision / recall. **Safety invariant: 0 false positives on contraindicated**, pinned in `tests/test_scripts/test_bitnet_confusion_matrix.py`. |
