...
| `Multi-seed FIM reproduction (V11 paper baseline; statistical-robustness validation)` | `tests/test_engine/test_multiseed_fim_reproduction_pin.py` (3 tests, iter-216) validates that the V11 paper's Pythia-6.9B + OLMoE-1B-7B Fisher-Information-Matrix benchmarks reproduce across separate Runpod runs within fp16 numerical-noise tolerance. Validation discipline: (1) **seed=42 reproduction** — Pythia-6.9B FIM at seed=42 must hit T1/T3 within 0.5% relative tolerance + Gini within 1e-3 absolute tolerance of the V11 paper baseline (`<research-project>`); same shape for `OLMoE-1B-7B-0924` (the **iter-216 wrong-model bug catch**: the V11 paper uses model `0924`, NOT the newer `0125` re-release — different model versions, T1/T3 differs ~38% — pin enforces correct model_id); (2) **paper-baseline file integrity** — the V11 paper baseline JSON files at the canonical <research-project> path must match the constants pinned in this test (drift fires the gate so a fresh-machine re-run of the V11 baselines requires a coordinated update). **Live measurement at iter-216**: Pythia seed=42 hit T1/T3 = 5737.60 vs paper 5738.42, Δ = 0.014% — well within fp16 tolerance, validates the multi-seed pipeline end-to-end. Tests skip when Runpod harvest hasn't landed (out-of-band timing); CI doesn't gate on multi-seed result presence. The **5-seed cluster** (seeds 42, 13, 256, 7, 99 on each model) lands incrementally on Runpod pods `8oexuui74c8pwi` (Pythia COMMUNITY) and `a63q2mmqipuwj5` (OLMoE-0924 COMMUNITY) and the pin auto-validates each seed against the paper baseline as it lands. |
