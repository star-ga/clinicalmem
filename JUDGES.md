...
2. **Recall gate (PCCP):** **100% recall** on every severity class
   represented in the 112-pair OpenEvidence-cited cohort —
   contraindicated (20/20), major (2/2), serious (68/68), moderate
...
#    major / serious / moderate against the 112-pair OpenEvidence-cited
#    cohort.
...
Optional: full pytest suite — `python3 -m pytest tests/test_engine/ tests/test_scripts/ -q`. Should report **977+ passed**.
...
|---|---|
| `100% recall · contraindicated · 20/20` | `docs/openevidence_cache.json` (112 entries) → `docs/pccp_eval_latest.json` (per-pair verdicts) |
...
| `21 synthetic patients · 38 NPIs` | `docs/synthea_demo_cohort.json` (FHIR R4 bundle, all NPIs Luhn-valid). **Per-patient drug-pair → cache-entry traceability matrix:** `docs/cohort_coverage_matrix.md` (regenerate with `scripts/build_cohort_coverage.py`). |
