...
2. **Recall gate (PCCP):** **100% recall** on every severity class
   represented in the 117-pair OpenEvidence-cited cohort —
...
#    major / serious / moderate against the 117-pair OpenEvidence-cited
#    cohort.
...
Optional: full pytest suite — `python3 -m pytest tests/test_engine/ tests/test_scripts/ -q`. Should report **1038+ passed**.
...
|---|---|
| `100% recall · contraindicated · 22/22` | `docs/openevidence_cache.json` (117 entries) → `docs/pccp_eval_latest.json` (per-pair verdicts) |
...
| `Layer 4.5 BitNet confusion matrix (live deployment)` | `docs/bitnet_confusion_matrix.json` (regenerate with `scripts/build_bitnet_confusion_matrix.py`) — full ground-truth × predicted matrix on the 117-pair cache, plus per-class precision / recall. **Safety invariant: 0 false positives on contraindicated**, pinned in `tests/test_scripts/test_bitnet_confusion_matrix.py`. |
...
| `26 synthetic patients · 43 NPIs` | `docs/synthea_demo_cohort.json` (FHIR R4 bundle, all NPIs Luhn-valid). **Per-patient drug-pair → cache-entry traceability matrix:** `docs/cohort_coverage_matrix.md` (regenerate with `scripts/build_cohort_coverage.py`). |
