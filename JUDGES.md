...
2. **Recall gate (PCCP):** **100% recall** on every severity class
   represented in the 119-pair OpenEvidence-cited cohort —
   contraindicated (24/24), major (4/4), serious (69/69), moderate
...
#    major / serious / moderate against the 119-pair OpenEvidence-cited
#    cohort.
...
| `28 synthetic patients · 45 NPIs` | `docs/synthea_demo_cohort.json` (FHIR R4 bundle, all NPIs Luhn-valid). **Per-patient drug-pair → cache-entry traceability matrix:** `docs/cohort_coverage_matrix.md` (regenerate with `scripts/build_cohort_coverage.py`). |
