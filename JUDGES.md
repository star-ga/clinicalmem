...
2. **Recall gate (PCCP):** **100% recall** on every severity class
   represented in the 127-pair OpenEvidence-cited cohort —
   contraindicated (30/30), major (4/4), serious (69/69), moderate
...
#    major / serious / moderate against the 127-pair OpenEvidence-cited
#    cohort.
...
Optional: full pytest suite — `python3 -m pytest tests/test_engine/ tests/test_scripts/ -q`. Should report **1139+ passed**.
...
|---|---|
| `100% recall · contraindicated · 32/32` | `docs/openevidence_cache.json` (127 entries) → `docs/pccp_eval_latest.json` (per-pair verdicts) |
