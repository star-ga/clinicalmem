...
2. **Recall gate (PCCP):** **100% recall** on every severity class
   represented in the 124-pair OpenEvidence-cited cohort —
   contraindicated (29/29), major (4/4), serious (69/69), moderate
...
#    major / serious / moderate against the 124-pair OpenEvidence-cited
#    cohort.
...
|---|---|
| `100% recall · contraindicated · 29/29` | `docs/openevidence_cache.json` (124 entries) → `docs/pccp_eval_latest.json` (per-pair verdicts) |
...
| `Layer 4.5 BitNet confusion matrix (live deployment)` | `docs/bitnet_confusion_matrix.json` (regenerate with `scripts/build_bitnet_confusion_matrix.py`) — full ground-truth × predicted matrix on the 124-pair cache, plus per-class precision / recall. **Safety invariant: 0 false positives on contraindicated**, pinned in `tests/test_scripts/test_bitnet_confusion_matrix.py`. |
