...
2. **Recall gate (PCCP):** **100% recall** on every severity class
   represented in the 121-pair OpenEvidence-cited cohort —
   contraindicated (26/26), major (4/4), serious (69/69), moderate
...
#    major / serious / moderate against the 121-pair OpenEvidence-cited
#    cohort.
...
Optional: full pytest suite — `python3 -m pytest tests/test_engine/ tests/test_scripts/ -q`. Should report **1090+ passed**.
...
|---|---|
| `100% recall · contraindicated · 26/26` | `docs/openevidence_cache.json` (121 entries) → `docs/pccp_eval_latest.json` (per-pair verdicts) |
...
| `8,512 ternary weights + 69 Q16.16 biases = 8,581 params / 19 KB` + bundle hash `cfadb4f6…` | `engine/bitnet_weights.json` + `engine/bitnet_classifier.py`. Param counts pinned by `tests/test_engine/test_bitnet_param_count_pin.py`. **Bundle-level integrity** independently pinned by `tests/test_engine/test_bitnet_bundle_integrity_pin.py` (8 tests: live `bundle_id` short prefix matches `cfadb4f6` AND tail matches `0b3f`; demo + JUDGES short-form citations match the live hash; file size stays within ±2 KB of 19 KB AND hard-ceiling 32 KB (Pi Zero 2 W edge claim); ternary-weight sparsity ≥ 40% (the iter-72 "structured sparsity" rhetoric); JSON key set is exactly `{_meta, hidden_b, hidden_w, output_b, output_w}`; `_meta` carries provenance fields; self-referenced `_meta.bundle_id` matches live SHA-256). |
| `Layer 4.5 BitNet confusion matrix (live deployment)` | `docs/bitnet_confusion_matrix.json` (regenerate with `scripts/build_bitnet_confusion_matrix.py`) — full ground-truth × predicted matrix on the 121-pair cache, plus per-class precision / recall. **Safety invariant: 0 false positives on contraindicated**, pinned in `tests/test_scripts/test_bitnet_confusion_matrix.py`. |
