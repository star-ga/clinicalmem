...
v8 pin's `_V8_CONTRA_HITS / _V8_CONTRA_TOTAL`.
...
The `_V8_CONTRA_HITS / _V8_CONTRA_TOTAL` constants in
`tests/test_engine/test_path_a_v8_live_recall_pin.py` are the single
...
JUDGES.md row 102 header (`Path A v8 — ...`) must contain:
  1. The recall fraction `<HITS>/<TOTAL>` matching live v8 pin constants
...
A future cohort growth that updates the v8 pin constants but forgets
...
def _live_v6_constants() -> dict[str, int]:
    """Extract _V8_CONTRA_HITS / _V8_CONTRA_TOTAL / _V8_FP_COUNT from
    the v8 pin source. Single source of truth: if the pin constants
    drift, this function reads the new values and the assertion below
    fires when JUDGES is stale."""
    text = _V8_PIN.read_text()
    out: dict[str, int] = {}
    for var in ("_V8_CONTRA_HITS", "_V8_CONTRA_TOTAL", "_V8_FP_COUNT",
                "_V8_MAJOR_HITS", "_V8_MAJOR_TOTAL"):
        m = re.search(rf"{var}\s*=\s*(\d+)", text)
        assert m is not None, (
            f"Could not find {var} in {_V8_PIN.relative_to(_REPO_ROOT)}. "
            f"This pin assumes _V6_*_HITS / _V6_*_TOTAL / _V8_FP_COUNT "
            f"are top-level int constants in the v8 live-recall pin."
        )
        out[var] = int(m.group(1))
    return out


def _v8_row_header() -> str:
    """Extract the JUDGES.md row 102 v8 header (first backtick-block
    after the row's pipe-prefix)."""
    text = _JUDGES.read_text()
    # Row format: `| `Path A v8 — ... XX/YY + ... 0 FP ...` | <body> |`
    ...
    assert m is not None, (
        ...
        "with '| `Path A v8 — ...` |'. If the row was renamed/moved, "
        "update this pin to find the new header."
    )
    return m.group(1)


def test_v8_header_contra_fraction_matches_live_pin():
    """The header's recall fraction must equal the live v8 pin's
    _V8_CONTRA_HITS / _V8_CONTRA_TOTAL — locks the iter-195/213/231
    header-vs-body drift class."""
    consts = _live_v6_constants()
    expected = f"{consts['_V8_CONTRA_HITS']}/{consts['_V8_CONTRA_TOTAL']}"
    header = _v8_row_header()
    ...
    matched = [(n, m) for n, m in fractions
               if n == str(consts["_V8_CONTRA_HITS"])
               and m == str(consts["_V8_CONTRA_TOTAL"])]
    assert matched, (
        f"JUDGES.md row 102 header missing live v6 contra fraction "
        f"{expected!r}. Header found: {header!r}\n"
        f"Live constants from {_V8_PIN.relative_to(_REPO_ROOT)}: "
        f"_V8_CONTRA_HITS={consts['_V8_CONTRA_HITS']}, "
        f"_V8_CONTRA_TOTAL={consts['_V8_CONTRA_TOTAL']}.\n"
...
def test_v8_header_major_fraction_matches_live_pin():
    ...
    catches up."""
    consts = _live_v6_constants()
    expected = f"{consts['_V8_MAJOR_HITS']}/{consts['_V8_MAJOR_TOTAL']}"
    header = _v8_row_header()
    fractions = re.findall(r"\b(\d+)/(\d+)\b", header)
    matched = [(n, m) for n, m in fractions
               if n == str(consts["_V8_MAJOR_HITS"])
               and m == str(consts["_V8_MAJOR_TOTAL"])]
    assert matched, (
        f"JUDGES.md row 102 header missing live v6 major fraction "
        f"{expected!r}. Header found: {header!r}\n"
        f"Live constants: _V8_MAJOR_HITS={consts['_V8_MAJOR_HITS']}, "
        f"_V8_MAJOR_TOTAL={consts['_V8_MAJOR_TOTAL']}."
    )


def test_v8_header_quotes_zero_fp_invariant():
    """The 0-false-positive invariant ("0 FP") is the load-bearing
    safety claim for v6 — must always appear in the header."""
    header = _v8_row_header()
    assert _live_v6_constants()["_V8_FP_COUNT"] == 0, (
        "v8 pin says zero FPs; if a FP lands the pin constants change "
...
def test_v8_header_quotes_q16_16_framing():
