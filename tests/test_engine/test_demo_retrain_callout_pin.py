...
def test_heatmap_footer_recall_is_correct():
    ...
    """
    text = _DEMO.read_text()
    assert "recall = 8 / 33 = 24%" in text, (
        "Heatmap footer recall must be 8/33 = 24% — iter-172 cohort "
    ...
    assert "recall = 8 / 32 = 25%" not in text  # iter-172 era stale (now 8/33)
...
def test_path_a_callout_iter140_numbers_not_stale():
    ...
    required_post_iter140 = (
        ...
        "33 / 33 contraindicated cache entries (100% explanation coverage)",  # iter-177 cohort 32 -> 33 (ketoconazole+ergotamine)
