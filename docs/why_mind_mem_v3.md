...
| `star-ga/mind-mem-4b` (the local fine-tune model) | ClinicalMem's LLM path is the 5-LLM US-based cloud-consensus surface (GPT-5.5 / Gemini 3.1 Pro / Grok 4.3 / Claude Opus 4.7 / Sonar Pro). The local fine-tune is an option for air-gapped deployments and is documented separately under "offline mode." |
