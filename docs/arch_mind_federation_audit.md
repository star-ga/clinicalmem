...
| 4 | Federation-invariant density | `flows/JointMemoryFederation.flow.mind` declares 21 typed runtime invariants; the bridge interacts with 16 of them through the demo's `egress` / `ingress` paths (the X25519-sealing invariants 17–21 await a dedicated MIC@2 federation-transport adapter targeting a future mind-mem release). 16/21 = 76% density. | ⚠️ Phase B target 80% — within margin pending the dedicated MIC@2 federation-transport adapter (mind-mem v3.11.1 is shipped and pinned but ships no new federation-transport module — the v3.10.x..v3.11.x line through v3.11.1 covers hook-installer + CLI + docs (v3.10.x) and quality-gate + typed-lineage + recall-explainability (v3.11.x); `http_transport.py` remains a single-workspace REST adapter for non-MCP clients, not p2p federation) |
...
(targeting a future mind-mem release) — a known boundary documented in
`docs/architecture.md`. Note: mind-mem v3.11.1 IS shipped and pinned (the
v3.10.x..v3.11.x line through v3.11.1 covers hook-installer + CLI + docs
(v3.10.x) and quality-gate + typed-lineage + recall-explainability
(v3.11.x)), but ships no new federation-transport module; its
`http_transport.py` remains a single-workspace REST adapter for non-MCP
clients (Slack bots / Streamlit dashboards), NOT a peer-to-peer federation
transport.**
