...
| **Mitigation 1** | 21 typed runtime invariants in `JointMemoryFederation.flow.mind` govern data movement across federation boundaries (16 exercised end-to-end by the mock demo `scripts/federation_mock_demo.py`; the 5 X25519-sealing invariants 17-21 are declared but await a dedicated MIC@2 federation-transport adapter targeting v4.0 "Platform Scale" per upstream ROADMAP.md — the v3.10.x..v3.12.x line through v3.12.0 shipped 2026-05-09 ships no new federation-transport module: v3.10.x is hook-installer + CLI + docs, v3.11.x adds quality-gate + typed-lineage + recall-explainability, and v3.12.x adds strict-quality-gate + lineage-staleness + red-team CI; federated recall + gRPC transport are explicitly v4.0 work, out of scope for v3.12); structural FHIR guards validate that no patient-identifiable fields cross site boundaries |
