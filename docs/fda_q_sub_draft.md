...
| **Mitigation 1** | 21 typed runtime invariants in `JointMemoryFederation.flow.mind` govern data movement across federation boundaries (16 exercised end-to-end by the mock demo `scripts/federation_mock_demo.py`; the 5 X25519-sealing invariants 17-21 are declared but await a dedicated MIC@2 federation-transport adapter targeting a future mind-mem release — the v3.10.x..v3.11.x line through v3.11.1 shipped 2026-05-08 ships no new federation-transport module: v3.10.x is hook-installer + CLI + docs only and v3.11.x adds quality-gate + typed-lineage + recall-explainability features); structural FHIR guards validate that no patient-identifiable fields cross site boundaries |
