...
| **Mitigation 1** | 21 typed runtime invariants in `JointMemoryFederation.flow.mind` govern data movement across federation boundaries — **all 21 exercised end-to-end** by the mock demo `scripts/federation_mock_demo.py` (the X25519-sealing invariants 10-14 are exercised in-process via `SealedEnvelope` + `_x25519_seal` / `_x25519_open` round-trip mirroring the v4 federation HTTP wire transport released in mind-mem v4.0.1 on PyPI 2026-05-11, commit `16a3e25` on `main` — 4 new endpoints in `src/mind_mem/http_transport.py` flag-gated by `v4.federation`, with stdlib `mind_mem.v4.federation_client.FederationClient`, 11/11 wire + 40/40 existing transport tests passing). mind-mem v4.0.0 — released 2026-05-10 — added the v4 federation **foundation** primitives (`mind_mem.v4.federation`: block_tier_vclock + tier_conflict_log + MergeStrategy enum) on top of the existing MemoryMesh + EventFanout control plane, plus cognitive-kernel + knowledge-graph + observability + resilience suites; structural FHIR guards validate that no patient-identifiable fields cross site boundaries |
...
| **Live federated transport** | The multi-site federation module ships as a typed contract specification with mind-mem v4.0.1 (federation foundation from v4.0.0 + HTTP wire transport from v4.0.1) wired through `engine/federation_transport.py`; live cross-site PHI exchange awaits ClinicalMem consumption of the v4 federation HTTP wire transport that released in mind-mem v4.0.1 on PyPI 2026-05-11 (commit `16a3e25` on `main`, 4 endpoints in `src/mind_mem/http_transport.py` flag-gated by `v4.federation`, stdlib `mind_mem.v4.federation_client.FederationClient`, 11/11 wire + 40/40 existing transport tests passing). | v2: released in mind-mem v4.0.1 on PyPI (2026-05-11) for ClinicalMem to consume; ClinicalMem `engine/federation_transport.py` bridge ready to wire through `FederationClient`. Group D follow-ups (gRPC/QUIC, TLS hardening, mTLS, OAuth/OIDC, DID/VC, ActivityPub bridge) deferred. |
