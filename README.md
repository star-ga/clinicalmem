...
<p align="center">
  <a href="#tests"><img src="https://img.shields.io/badge/tests-984%20passed-brightgreen?style=flat-square" alt="Tests"></a>
...
| **Test coverage** | 984+ tests across engine + scripts; line coverage measured per-suite via `pytest --cov` (not enforced as headline) | Untested |
...
├── .github/workflows/
│   ├── test.yaml               # CI: Run tests on push (984+ tests across engine + scripts)
...
**984 tests** across `tests/test_engine/`, `tests/test_scripts/`,
...
| **Test suite execution** | ~40 s for 984+ engine + script tests (full suite ~5 min including a2a/mcp) |
...
- &check; 984+ automated tests including adversarial cases (negation detection, boundary conditions)
