...
> **Path A v8 — full-recall breakthrough (iter 244, staged but not yet engine-promoted).** A retrained 193-dim × **256-hidden** ternary bundle (`retrain_runpod/bitnet_weights_v8_h256.json`, `bundle_id 1f0f8859…`) with 9-anchor BOOST_KEYS @200× upweighting hits **43 / 43 contraindicated (100%) + 4 / 4 major (100%) + 0 FP** under cross-arch Q16.16 inference on the iter-235 41-contra live cache — **zero known misses**. The doubled hidden_dim 128 → 256 broke the v7 architectural ceiling discovered at iter-241 (where v7 at h=128 couldn't simultaneously satisfy 41/41 + 4/4 + 0 FP regardless of seed; best v7 was 40/41+4/4+0FP, matching predecessor v6). v8 has 50,688 ternary weights vs v6's 25,344 (~38 KB bundle, still <1 ms/pair on Pi Zero — same Q16.16 pure-integer math at 2× capacity). Predecessor v6 (40/41, h=128, `592ee51e…`) kept on disk for FDA SaMD audit-trail rigor. Engine promotion of v8 requires the 4–6 h cascade refactor (encoder lift to 193-dim + JS bit-identity mirror + audit-replay regen + manifest SHA rotation + 46 audit-pin re-replay) **plus** the 64 → 256 `hidden_w` shape extension in `engine/bitnet_classifier.py`. The v8 bundle is pinned at TWO levels — `tests/test_engine/test_path_a_v8_live_recall_pin.py` (6 aggregate tests: bundle_id + 43/43 contra + 4/4 major + 0 FP + meta-block invariants + _V8_EXPECTED_MISSES empty-tuple invariant + strictly-supersedes-v6 invariant) and `tests/test_engine/test_path_a_v8_q16_determinism_pin.py` (8 per-pair tests: 16 canonical pairs × 4 pinned values + 100×16 = 1600 forward-pass determinism stress + cross-pin invariant locking the BOOST_KEYS promise — every prior v5 historical-miss + iter-215 lurasidone+ketoconazole v6-known-miss are ALL classified contraindicated under v8) — so it cannot drift before promotion.
