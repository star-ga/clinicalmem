...
<p align="center">
  <a href="#tests"><img src="https://img.shields.io/badge/tests-898%20passed-brightgreen?style=flat-square" alt="Tests"></a>
...
| **Test coverage** | 898+ tests across engine + scripts; line coverage measured per-suite via `pytest --cov` (not enforced as headline) | Untested |
...
├── .github/workflows/
│   ├── test.yaml               # CI: Run tests on push (898+ tests across engine + scripts)
...
**898 tests** across `tests/test_engine/`, `tests/test_scripts/`,
...
| **Test suite execution** | ~40 s for 898+ engine + script tests (full suite ~5 min including a2a/mcp) |
...
- &check; 898+ automated tests including adversarial cases (negation detection, boundary conditions)
