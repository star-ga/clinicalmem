...
<p align="center">
  <a href="#tests"><img src="https://img.shields.io/badge/tests-1274%20passed-brightgreen?style=flat-square" alt="Tests"></a>
...
> **Path A v6 — generalization advance (iter 207, staged but not yet engine-promoted).** A retrained 193-dim × **128-hidden** ternary bundle (`retrain_runpod/bitnet_weights_v6_h128.json`, `bundle_id 592ee51e…`) with 8-anchor BOOST_KEYS @200× upweighting hits **40 / 41 contraindicated (97.6%) + 4 / 4 major (100%) + 0 FP** under cross-arch Q16.16 inference on the iter-235 41-contra live cache. The 1 known miss (`lurasidone + ketoconazole`, atypical antipsychotic CYP3A4 substrate added at iter-215) is queued for v7 retrain with extended BOOST_KEYS — v6's iter-207 upweighting trained drug-pair-specific recognition rather than full sub-class generalization. v6 closes the cohort-growth gap left by prior staged bundles. Engine promotion of v6 requires the 4–6 h cascade refactor (encoder lift to 193-dim + JS bit-identity mirror + audit-replay regen + manifest SHA rotation + 44 audit-pin re-replay) **plus** the 64 → 128 `hidden_w` shape extension in `engine/bitnet_classifier.py`. The v6 bundle is pinned at TWO levels — `tests/test_engine/test_path_a_v6_live_recall_pin.py` (6 aggregate tests: bundle_id + 40/41 contra + 4/4 major + 0 FP + meta-block invariants + _V6_EXPECTED_MISSES exact-equality + strictly-supersedes-v5 invariant) and `tests/test_engine/test_path_a_v6_q16_determinism_pin.py` (8 per-pair tests: 16 canonical pairs × 4 pinned values + 100×16 = 1600 forward-pass determinism stress + cross-pin invariant locking the BOOST_KEYS promise) — so it cannot drift before promotion.
...
**1274 tests** across `tests/test_engine/`, `tests/test_scripts/`,
