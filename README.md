...
<p align="center">
  <a href="#tests"><img src="https://img.shields.io/badge/tests-888%20passed-brightgreen?style=flat-square" alt="Tests"></a>
...
| **Test coverage** | 888+ tests across engine + scripts; line coverage measured per-suite via `pytest --cov` (not enforced as headline) | Untested |
...
├── .github/workflows/
│   ├── test.yaml               # CI: Run tests on push (888+ tests across engine + scripts)
...
**888 tests** across `tests/test_engine/`, `tests/test_scripts/`,
...
| **Test suite execution** | ~40 s for 888+ engine + script tests (full suite ~5 min including a2a/mcp) |
...
- &check; 888+ automated tests including adversarial cases (negation detection, boundary conditions)
