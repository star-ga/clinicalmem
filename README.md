...
<p align="center">
  <a href="#tests"><img src="https://img.shields.io/badge/tests-1425%20passed-brightgreen?style=flat-square" alt="Tests"></a>
...
> **What Layer 4.5 actually is.** A bit-identical reproducibility primitive AND, post-iter-421 ensemble shipping, a full-recall safety classifier on **every** severity class. Bit-identical Q16.16 inference across CPU/GPU/NPU is the load-bearing FDA SaMD audit-replay property; the iter-421 Path B 2-bundle ensemble cascade (frozen v8 bundle A → tier-2 specialist bundle B under constrained argmax) closes the v8-standalone serious / moderate gap (84% → 100% / 91% → 100%) and lifts every class to 100% recall on the 139-pair live cohort: **44/44 contraindicated · 4/4 major · 69/69 serious · 22/22 moderate · 0 FP**. Upstream Layers 1–4 (RxNorm + OpenEvidence + NIH RxNav + 6-LLM consensus) still provide independent corroboration. The audit chain records every BitNet decision under a composite `weights_id = "{bundle_id_a}+{bundle_id_b}"` so any auditor with both bundles (~150 KB combined) can replay any ensemble decision decades later, no proprietary toolchain required.

> **iter-421 Path B 2-bundle ensemble — 100% recall on every severity class, LIVE in engine.** Multi-LLM consensus (4-of-5: DeepSeek/Mistral/GLM/NVIDIA-Llama70B) recipe: **frozen v8 contra/major gate (bundle A, `1f0f8859…`, 193-dim × 256-hidden, 50,688 ternary weights, ~118 KB) cascades to a tier-2 specialist (bundle B, `5f7ed5f6…`, 193-dim × 64-hidden, ~12K ternary weights, ~30 KB) trained EXCLUSIVELY on the 95 non-contra samples.** Engine dispatcher: A predicts contraindicated → return contraindicated; otherwise B's constrained argmax over `{moderate, serious, major}` replaces A's serious-class call. Result on 139-pair PCCP cohort: **44/44 contra · 4/4 major · 69/69 serious · 22/22 moderate · 0 contra FP · 0 major FP**. Composite `weights_id = "{a_id}+{b_id}"` audit primitive captured in every `BitNetResult.repro_hash` so cascade decisions are byte-replayable across architectures. Bit-identical JS browser mirror at `docs/bitnet_browser.js` (Verify Replay button runs the same cascade in-browser). Three single-model retrain attempts (v9 / v10 / v11) all regressed contra recall by 3-8 anchors — the empirical conclusion is 193-dim feature space cannot represent 4-class fine discrimination simultaneously, which is why iter-421 splits the problem into disjoint single-class-domain specialists. The pre-iter-421 v8 standalone baseline (84% serious, 91% moderate) is preserved at `retrain_runpod/bitnet_weights_v8_h256.json`; the cfadb4f6 v1 baseline at `engine/bitnet_weights.v1.cfadb4f6.bak.json` for full audit-chain reconstruction. v8 is pinned at TWO levels — `tests/test_engine/test_path_a_v8_live_recall_pin.py` (bundle_id + 44/44 contra + 4/4 major + 0 FP + strictly-supersedes-v6 invariant) and `tests/test_engine/test_path_a_v8_q16_determinism_pin.py` (18 canonical pairs × 4 pinned values + 1800 forward-pass determinism stress).
...
| **Test coverage** | 1425+ tests across engine + scripts; line coverage measured per-suite via `pytest --cov` (not enforced as headline) | Untested |
...
├── .github/workflows/
│   ├── test.yaml               # CI: Run tests on push (1425+ tests across engine + scripts)
...
**1425 tests** across `tests/test_engine/`, `tests/test_scripts/`,
