...
<p align="center">
  <a href="#tests"><img src="https://img.shields.io/badge/tests-1377%20passed-brightgreen?style=flat-square" alt="Tests"></a>
...
| 4.5 | **BitNet b1.58 reproducibility primitive (Path A v8 — engine-promoted iter 275)** | **Full-recall safety classifier + audit-replay anchor.** 50,688 ternary weights + 261 Q16.16 biases (50,949 total parameters), 193-dim hash + ATC + pair-derived encoder × 256-hidden architecture; **bit-identical Q16.16 forward pass across CPU/GPU/NPU**. **100.0% live deployment precision AND 100.0% recall on `contraindicated`** (44 / 44 of its contraindicated predictions are correct on the 44-pair live cache, zero false negatives, zero false positives) + **100.0% major recall** (4/4). Every output carries a SHA-256 `repro_hash` any auditor can re-verify in `<1 ms`. Trained on the v8 augmented corpus with 9-anchor BOOST_KEYS @200×. | < 1ms |
| 5 | **LLM Synthesis** | Evidence-cited clinical explanations | ~3s |
| 6 | **Abstention Gate** | "I don't know" when evidence insufficient | 0ms |

> **What Layer 4.5 actually is.** A bit-identical reproducibility primitive AND, post-iter-275 v8 promotion, a full-recall safety classifier on the contraindicated and major classes. Bit-identical Q16.16 inference across CPU/GPU/NPU is the load-bearing FDA SaMD audit-replay property; v8's 193-dim hash + 26 ATC flag + 13 pair-derived encoder × 256-hidden architecture closes the iter-235 generalization gap and lifts contra recall to **100% (44/44, 0 FP)** + major recall to **100% (4/4)**. Upstream Layers 1–4 (RxNorm + OpenEvidence + NIH RxNav + 6-LLM consensus) still carry primary recall on serious + moderate; Layer 4.5 disagreements still trigger `BITNET_SAFETY_DOWNGRADE_DISAGREEMENT` alerts and the safer verdict always wins. The audit chain records every BitNet decision and any auditor with the ~118 KB v8 weights bundle can replay any decision decades later, no proprietary toolchain required.

> **Path A v8 — full-recall breakthrough, LIVE in engine (promoted iter 275).** The 193-dim × **256-hidden** ternary bundle (`engine/bitnet_weights.json`, `bundle_id 1f0f8859…`) with 9-anchor BOOST_KEYS @200× upweighting hits **44 / 44 contraindicated (100%) + 4 / 4 major (100%) + 0 FP** under cross-arch Q16.16 inference on the live 139-pair cache — **zero known misses**. The doubled hidden_dim 128 → 256 broke the v7 architectural ceiling at iter-241; v8 has 50,688 ternary weights / 261 Q16.16 biases (50,949 total params, ~118 KB bundle, still <1 ms/pair on Pi Zero — same Q16.16 pure-integer math at ~6× the v1 capacity). The pre-promotion v1 baseline (`cfadb4f6`, hash-only 128-dim × 64-hidden) is preserved at `engine/bitnet_weights.v1.cfadb4f6.bak.json` for full audit-chain reconstruction; the v6 staged predecessor (40/41, h=128, `592ee51e…`) also kept on disk for FDA SaMD audit-trail rigor. The iter-275 promotion cascade landed: 193-dim encoder lifted (`engine/bitnet_features_v8.py`, bit-identical with trainer 6/6 canonical pairs), JS bit-identity mirror restored at iter-276 (in-browser BitNet matches Python byte-for-byte under v8), audit-replay regenerated under the v8 bundle_id, manifest SHA rotated, severity vocabulary corpus-aligned to `(none, moderate, serious, major, contraindicated)`, 26 test pin files updated. v8 is pinned at TWO levels — `tests/test_engine/test_path_a_v8_live_recall_pin.py` (6 aggregate tests: bundle_id + 44/44 contra + 4/4 major + 0 FP + meta-block invariants + _V8_EXPECTED_MISSES empty-tuple invariant + strictly-supersedes-v6 invariant) and `tests/test_engine/test_path_a_v8_q16_determinism_pin.py` (8 per-pair tests: 18 canonical pairs × 4 pinned values + 100×18 = 1800 forward-pass determinism stress + cross-pin invariant locking the BOOST_KEYS promise — every prior v5 historical-miss + iter-215 lurasidone+ketoconazole v6-known-miss are ALL classified contraindicated under v8).
...
| **Test coverage** | 1377+ tests across engine + scripts; line coverage measured per-suite via `pytest --cov` (not enforced as headline) | Untested |
  ...
  "findings": [
    {
      ...
      "sources": ["Deterministic table", "OpenEvidence (Mayo Clinic)", "NIH Drug Interaction API", "6/6 LLM consensus (US-based)"]
    }
  ],
  "total_findings": 1,
...
├── .github/workflows/
│   ├── test.yaml               # CI: Run tests on push (1377+ tests across engine + scripts)
...
**1377 tests** across `tests/test_engine/`, `tests/test_scripts/`,
...
tests/test_engine/         — 600+ tests covering the safety pipeline:
   ...
   federation transport bridge (mind-mem v3.10.2 MemoryMesh +
...
| **Test suite execution** | ~40 s for 1373+ engine + script tests (full suite ~5 min including a2a/mcp) |
...
- &check; 1373+ automated tests including adversarial cases (negation detection, boundary conditions)
