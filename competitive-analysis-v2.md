...
- 6 layers of clinical safety (was 4 in March)
...
- 235+ tests (CHANGELOG / DEVPOST cite 356)
...
| **Feasibility** (regulatory, deployable, real) | High | **Live cloud deployment, SHA-256 hash-chain audit, abstention gate, 235+ tests, MIT license, MCP+A2A both, PHI guard, hallucination detection** | Docker Compose only; localhost; no audit; no abstention; no tests | 21 commits; no audit; no compliance docs | Clean architecture but shallow features |
...
| ID | Item | Time | Why |
|---|---|---|---|
| **P2.1** | Push tests from 235 → 400+. Coverage targets: MCP tool round-trip, A2A skill round-trip, every NIH RxNorm error path, every FHIR fallback, every abstention boundary | 6 hrs | "Feasibility" criterion weights tests heavily. 400+ is a clear lead. |
...
- *"Feasibility"* — live deployment, audit trail, abstention, 235+ tests, MIT license, MCP + A2A both. Every other entry has at least one missing pillar.
