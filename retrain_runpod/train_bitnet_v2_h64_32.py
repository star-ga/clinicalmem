...
WEIGHTS_OUT = _CLINICALMEM / "retrain_runpod" / "bitnet_weights_v2_h64_32.json"
...
class BitNetClassifier(nn.Module):
    def __init__(self):
        super().__init__()
        # Option 3: two hidden layers (64 → 32) for deeper non-linear
        # combinations of the same 128-dim ternary feature input.
        self.hidden1 = TernaryLinear(128, 64)
        self.hidden2 = TernaryLinear(64, 32)
        self.output = TernaryLinear(32, 5)
...
def export_weights(model, path):
    """Option-3 export: two hidden layers (h1 64, h2 32). Stored under
    distinct keys hidden1_w / hidden2_w / output_w; engine loader does
    not know about this shape (validates single-hidden 64×128). For
    standalone evaluation only."""
    model = model.cpu().eval()
    with torch.no_grad():
        h1_w_t = ternary_quantize(model.hidden1.weight).to(torch.int8)
        h2_w_t = ternary_quantize(model.hidden2.weight).to(torch.int8)
        output_w_t = ternary_quantize(model.output.weight).to(torch.int8)
        h1_b = model.hidden1.bias
        h2_b = model.hidden2.bias
        output_b = model.output.bias

    payload = {
        "hidden1_w": [[int(v) for v in row] for row in h1_w_t.tolist()],
        "hidden1_b": [_to_q16(float(v)) for v in h1_b.tolist()],
        "hidden2_w": [[int(v) for v in row] for row in h2_w_t.tolist()],
        "hidden2_b": [_to_q16(float(v)) for v in h2_b.tolist()],
        "output_w": [[int(v) for v in row] for row in output_w_t.tolist()],
        "output_b": [_to_q16(float(v)) for v in output_b.tolist()],
        "_meta": {
            "schema": "bitnet_classifier_v2_two_hidden",
            "in_features": 128,
            "hidden1_features": 64,
            "hidden2_features": 32,
            "out_features": 5,
            ...
            "training_iter": "iter-95-option3",
            "augmentation": "cache_contraindicated_anchors_x_200",
        },
    }
    canonical = json.dumps(
        {k: payload[k] for k in ("hidden1_w", "hidden1_b", "hidden2_w", "hidden2_b", "output_w", "output_b")},
...
def main() -> int:
    ...
    if not passed:
        ...
        print("  Do NOT promote bitnet_weights_v2_h64_32.json to engine/bitnet_weights.json.")
