...
- **Trained BitNet b1.58 reproducibility classifier** — Layer 4.5 is a real model, not a stub. Trained on a 3,247-pair clinical-pharmacology corpus across 224 medications and 41 drug classes (from the deterministic table + ~50 hand-coded class-pair rules covering bleeding axis, serotonin syndrome, CNS depression, statin metabolism, hyperkalemia axis, cardiac conduction, renal/metabolic risk, and 14 specific contraindications). Held-out test accuracy: **68.6% macro / 85.7% on the load-bearing `contraindicated` class**. Total parameters: 8,517 ternary {-1, 0, +1} weights — serializes to 19 KB JSON, runs in `<1 ms` per pair on a $5 Raspberry Pi Zero, bit-identical to the same forward pass on an RTX 3080 or Apple M-series NPU. Full training recipe + corpus build script + reproducibility hashes documented in `docs/bitnet_training.md`.
